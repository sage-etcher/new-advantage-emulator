...
{                                                                             \
        (x) = mobo_ram_read (context, (address) & 0xffff);                    \
}
...
{                                                                             \
        (x) = (mobo_ram_read (context, (address) & 0xffff)                    \
            | (mobo_ram_read (context, ((address) + 1) & 0xffff) << 8));      \
}
...
{                                                                             \
        mobo_ram_write (context, ((address) & 0xffff), (x));                  \
}

#define Z80_WRITE_WORD(address, x)                                            \
{                                                                             \
        mobo_ram_write (context, (address) & 0xffff, (x));                    \
        mobo_ram_write (context, ((address) + 1) & 0xffff, (x) >> 8);         \
}
...
{                                                                             \
        mobo_input_byte (context, (port), (x));                               \
}

#define Z80_OUTPUT_BYTE(port, x)                                              \
{                                                                             \
        (x) = mobo_output_byte (context, (port));                             \
}
